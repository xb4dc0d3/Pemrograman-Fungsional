...
loopPrinter = do 
    ...
    let lambda_church_notation = (reduceNF parsedString)!!((length (reduceNF parsedString))-1)

    --- Concat the backslash '/' to '//' using show string---
    putStrLn ("==> Lambda church notation: "++ show lambda_church_notation)
